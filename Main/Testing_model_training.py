...
# 1st conv layer
model.add(tf.keras.layers.Conv1D(128, kernel_size=3, activation='relu', input_shape=(num_features, 1), padding='same', kernel_regularizer=tf.keras.regularizers.l2(0.001)))
model.add(tf.keras.layers.BatchNormalization())
model.add(tf.keras.layers.MaxPooling1D(pool_size=2, strides=2, padding='same'))

# 2nd conv layer
model.add(tf.keras.layers.Conv1D(256, kernel_size=3, activation='relu', padding='same', kernel_regularizer=tf.keras.regularizers.l2(0.001)))
model.add(tf.keras.layers.BatchNormalization())
model.add(tf.keras.layers.MaxPooling1D(pool_size=2, strides=2, padding='same'))
...
model.add(tf.keras.layers.Dense(512, activation='relu'))  # Dense layer with 512 units
